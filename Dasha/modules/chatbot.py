...
from Dasha import BOT_USERNAME as bu
from Dasha import BOT_ID, pbot, arq
from Dasha.ex_plugins.chatbot import add_chat, get_session, remove_chat
from Dasha.utils.pluginhelper import admins_only, edit_or_reply
...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Dasha AI Already Activated In This Chat")
            return
        await lel.edit(f"Dasha AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("Dasha AI Was Not Activated In This Chat")
            return
        await lel.edit(f"Dasha AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("Dasha", "Aco")
        test = test.replace("Dasha", "Aco")
        test = test.replace("My god is @NickyLrca", "I'm a Christian")
        test = test.replace("16", "9")
        test = test.replace("@NickyLrca is my husband.", "I'm single.")
        test = test.replace("My husband is @NickyLrca", "I'm single.")
        test = test.replace("@machinaxsupport", "Dashabot.ai")
        ...
        test = test.replace(
            "Hello there I am Dasha...nice to meet u",
            "Hi, my friend! Do you want me to tell you a joke?")
        test = test.replace("@NickyLrca is my owner", "Have the control right.")
        test = test.replace(
            "Hi, My name is Dasha Nice to meet you.",
            "Hi, my friend, what can I do for you today?")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "Dasha")
        response = response.replace("aco", "Dasha")
        response = response.replace("Luna", "Dasha")
        response = response.replace("luna", "Dasha")
        ...
        response = response.replace("Dashabot.ai", "@machinaxsupport")
...
@pbot.on_message(filters.regex("Dasha|Dasha|robot|Dasha|vain") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
